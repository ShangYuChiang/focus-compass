...
describe("daily review card", () => {
  it("使用產卡快照，不會被日後任務改名影響", () => {
    ...
    expect(dailyReviewDisplay(review(), state).mvpTitle).toBe("完成履歷三項成果");
  });
  ...
  it("舊收工卡沒有快照時會由現有資料補出統計", () => {
    const state = createInitialState();
    state.sessions = [{
      id: "s1", taskId: "t-career-1", axisId: "career", startedAt: "2026-08-19T10:00:00",
    ...
    const oldReview: DailyReview = { date: "2026-08-19", mvpTaskId: "t-career-1", tomorrowAxisId: "career" };
